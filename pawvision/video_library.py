...
class VideoLibraryManager:
    ...
    def __init__(
        self,
        db_path: str = "pawvision.db",
        video_directories: List[str] = None,
        youtube_cache_dir: str = "youtube_cache",
        youtube_preferred_quality: str = "720p",
    ):
    ...
    def sync_with_filesystem(self, video_paths: List[str], duration_getter_func) -> Tuple[int, int, int]:
        ...
        try:
            ...
            for video_path in video_paths:
                ...
                try:
                    ...
                    if existing_entry is None:
                        ...
                        or existing_entry.size != file_size
                        or existing_entry.duration is None
                    ):
            ...
            if added_count > 0 or updated_count > 0 or removed_count > 0:
                self.logger.info(
                    "Library sync completed: %d added, %d updated, %d removed",
                    added_count,
                    updated_count,
                    removed_count,
                )
