    ...
    def _register_routes(self):
        ...
        def api_video_library():
            ...
            try:
                ...
                return (
                    jsonify({"success": False, "message": "Failed to get video library"}),
                    500,
                )
                ...
                if video_path.startswith("youtube://"):
                    ...
                    if video_entry and video_entry.download_path and os.path.exists(video_entry.download_path):
                        try:
                            ...
                            self.logger.error(
                                "Failed to delete downloaded file %s: %s",
            ...
            try:
                ...
                if custom_start_time:
                    try:
                        ...
                        if start_time < 0:
                            return (
                                jsonify({"error": "Start time cannot be negative"}),
                                400,
                            )
                        if start_time >= video_entry.duration:
                            return (
                                jsonify({"error": "Start time cannot be greater than video duration"}),
                                400,
                            )
                    except ValueError:
                ...
                if custom_end_offset:
                    try:
                        ...
                        if end_offset <= 0:
                            return (
                                jsonify({"error": "End offset must be positive"}),
                                400,
                            )
                        if end_offset >= video_entry.duration:
                            return (
                                jsonify({"error": "End offset cannot be greater than video duration"}),
                                400,
                            )
                        ...
                        if start_time is not None and end_time <= start_time:
                            return (
                                jsonify({"error": "End time must be after start time"}),
                                400,
                            )
                    except ValueError:
        ...
        def download_youtube():
            ...
            try:
                ...
                if not video_entry or not video_entry.is_youtube:
                    return (
                        jsonify({"error": "Video not found or not a YouTube video"}),
                        400,
                    )
        ...
        def validate_youtube_url():
            ...
            try:
                ...
                if title:
                    return (
                        jsonify({"valid": True, "title": title, "duration": duration}),
                        200,
                    )
                else:
                    return (
        ...
        def get_download_progress(download_id):
            ...
            try:
                progress = self.download_progress.get(
                    download_id,
                    {
                        "status": "not_found",
                        "percentage": 0,
        ...
        @self.app.route("/api/current-video")
        def api_current_video():
            """API endpoint to get currently playing video information."""
            try:
                current_video = self.video_player.get_current_video_info()
                ...
                self.logger.error("API current video error: %s", e)
                return jsonify({"error": "Internal error"}), 500
        ...
        def api_statistics():
            ...
            try:
                if not self.statistics_manager:
                    return (
                        jsonify({"status": "error", "message": "Statistics not enabled"}),
                        404,
                    )
        ...
        def api_clear_statistics():
            ...
            try:
                ...
                return (
                    jsonify({"status": "success", "message": "Statistics cleared"}),
                    200,
                )
