...
class ButtonHandler:
    ...
    def _init_motion_sensor(self):
        ...
        try:
            ...
            self.logger.info(
                "Motion sensor initialized on GPIO pin %d",
                self.config.motion_sensor_pin,
            )
    ...
    def _handle_button_press(self):
        ...
        if self.video_player.is_playing():
            ...
            if self.statistics_manager:
                button_allowed = self.statistics_manager.record_button_press(
                    self.config.button_cooldown_seconds, is_interruption=False
                )
    ...
    def is_button_allowed(self) -> bool:
        ...
        in_disable_range = time_parser.is_time_in_range(
            self.config.button_disable_start, self.config.button_disable_end
        )
...
class Scheduler:
    ...
    def start(self):
        ...
        self.running = True
        self.scheduler_thread = threading.Thread(target=self._scheduler_loop, daemon=True, name="PawVision-Scheduler")
...
class GPIOManager:
    ...
    def __init__(self, config, video_player, statistics_manager=None):
        ...
        self.button_handler = ButtonHandler(config, video_player, statistics_manager)
        self.scheduler = Scheduler(config, video_player, statistics_manager)
