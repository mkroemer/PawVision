...
class VideoPlayer:
    ...
    def __init__(self, config, video_dirs, statistics_manager=None, monitor_manager=None):
        ...
        # Initialize cache
        self.cache = VideoCache(config.cache_file, config.enable_duration_cache)
    ...
    def sync_video_library(self):
        ...
        added, updated, removed = self.library_manager.sync_with_filesystem(video_files, self.get_video_duration)
    ...
    def get_current_video_info(self) -> Optional[dict]:
        ...
        with self.process_lock:
            if not self.is_playing() or self.current_video is None:
                return None
            ...
            # Calculate playback time if available
            playback_time = None
            if self.video_start_time:
                playback_time = (datetime.now() - self.video_start_time).total_seconds()
            ...
            return {
                ...
                "started_at": self.video_start_time.isoformat() if self.video_start_time else None
            }
        ...
        with self.process_lock:
            if self.current_process and self.current_process.poll() is None:
                ...
                if self.statistics_manager and self.current_video and viewing_duration:
                    self.statistics_manager.record_video_viewing(self.current_video, viewing_duration, reason)
    ...
    def _stop_after_timeout(self, timeout_sec: int):
        ...
        with self.process_lock:
            if self.current_process and self.current_process.poll() is None:
                ...
                # Record viewing statistics
                if self.statistics_manager and self.current_video and viewing_duration:
                    self.statistics_manager.record_video_viewing(self.current_video, viewing_duration, "timeout")
                ...
                if entry.is_youtube:
                    ...
                    info = {
                        ...
                        "effective_duration_str": (
                            self._format_duration(entry.get_effective_duration())
