...
class PawVisionApp:
    ...
    def initialize(self):
        ...
        try:
            ...
            self.video_player = VideoPlayer(self.config, video_dirs, self.statistics_manager)
            ...
            self.gpio_manager = GPIOManager(self.config, self.video_player, self.statistics_manager)
    ...
    def start(self):
        ...
        try:
            ...
            if self.dev_mode:
                # In development mode, run Flask directly with debug mode for hot reload
                self.running = True
...
def main():
    ...
    try:
        # In dev mode, Flask runs in main thread and blocks
        # In production mode, we need to keep the main thread alive
        if not dev_mode:
            app.run_forever()
